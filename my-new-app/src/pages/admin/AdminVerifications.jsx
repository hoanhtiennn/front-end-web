...
import { Eye, CheckCircle2, XCircle, X, RefreshCw, AlertCircle } from "lucide-react";

const STATUS_CONFIG = {
  PENDING:        { label: "Chờ duyệt (Auto)",   cls: "bg-yellow-100 text-yellow-800 border border-yellow-200" },
  PENDING_MANUAL: { label: "Chờ duyệt (Thủ công)", cls: "bg-orange-100 text-orange-800 border border-orange-200" },
  APPROVED:       { label: "Đã duyệt",            cls: "bg-green-100 text-green-800 border border-green-200" },
  REJECTED:       { label: "Từ chối",             cls: "bg-red-100 text-red-800 border border-red-200" },
};

function StatusBadge({ status }) {
  const cfg = STATUS_CONFIG[status] || { label: status, cls: "bg-gray-100 text-gray-700 border border-gray-200" };
  return (
...
export default function AdminVerifications() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      else if (Array.isArray(res.data?.result?.content)) data = res.data.result.content;
  ...
  const handleApprove = (id) => {
    showConfirm({
      ...
      message: "Bạn chắc chắn muốn DUYỆT yêu cầu xác minh này? Hành động này sẽ cập nhật trạng thái của Landlord.",
      variant: "success",
      onConfirm: async () => {
        try {
          setActionLoading(true);
          await axios.put(`/api/verifications/${id}/approve`, {}, {
            headers: { Authorization: `Bearer ${token()}` },
          });
          setSelectedReq(null);
          fetchAll();
  ...
  const handleReject = async (id) => {
    ...
    try {
      setActionLoading(true);
      await axios.put(`/api/verifications/${id}/reject`, { reason: rejectReason }, {
        headers: { Authorization: `Bearer ${token()}` },
      });
      setSelectedReq(null);
      setRejectReason("");
  ...
  const pendingCount  = requests.filter(r => r.status === "PENDING" || r.status === "PENDING_MANUAL").length;
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Quản lý Xác minh CCCD</h2>
          <p className="text-sm text-gray-500 mt-1">Kiểm duyệt giấy tờ tùy thân của chủ trọ</p>
        </div>
        <button
          onClick={fetchAll}
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Chờ duyệt", count: pendingCount,  color: "text-yellow-700", bg: "bg-yellow-50 border-yellow-200" },
          { label: "Đã duyệt",  count: approvedCount, color: "text-green-700",  bg: "bg-green-50 border-green-200" },
          { label: "Từ chối",   count: rejectedCount, color: "text-red-700",    bg: "bg-red-50 border-red-200" },
        ].map(s => (
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan="6" className="px-6 py-10 text-center text-gray-400">Không có yêu cầu nào</td></tr>
            ) : (
              requests.map(r => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-gray-900">{r.userName || "—"}</div>
                    <div className="text-xs text-gray-500 mt-0.5">{r.userEmail || "—"}</div>
                  </td>
                  <td className="px-6 py-4 text-gray-700 font-medium">{r.extractedName || "Chưa đọc"}</td>
                  <td className="px-6 py-4 text-gray-500 text-xs">{formatDate(r.createdAt)}</td>
                  <td className="px-6 py-4 text-center"><StatusBadge status={r.status} /></td>
                  <td className="px-6 py-4 text-center">
                    {r.autoVerified ? (
                      <span className="text-xs font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full border border-green-200">Tự động</span>
                    ) : (
                      <span className="text-xs font-bold text-gray-500 bg-gray-50 px-2 py-0.5 rounded-full border border-gray-200">Thủ công</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="inline-flex items-center gap-1.5">
                      <button
                        onClick={() => { setSelectedReq(r); setShowRejectInput(false); setRejectReason(""); }}
                        title="Xem chi tiết"
                        className="p-1.5 text-gray-500 hover:text-black hover:bg-gray-100 rounded-lg transition-colors"
                      >
                        <Eye size={16} />
                      </button>
                      {(r.status === "PENDING" || r.status === "PENDING_MANUAL") && (
                        <>
                          <button
                          ...
                          <button
                            onClick={() => { setSelectedReq(r); setShowRejectInput(true); }}
                            title="Từ chối"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                <h3 className="font-bold text-lg text-gray-900">Chi tiết Xác minh</h3>
                <p className="text-xs text-gray-500 mt-0.5">{selectedReq.userEmail}</p>
              </div>
              <button
                onClick={() => { setSelectedReq(null); setShowRejectInput(false); setRejectReason(""); }}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  { label: "Họ tên (OCR)",   value: selectedReq.extractedName   || "—" },
                  { label: "Số CCCD (OCR)",   value: selectedReq.extractedIdNumber || "—" },
                  { label: "Trạng thái",      value: <StatusBadge status={selectedReq.status} /> },
                  { label: "Confidence",      value: selectedReq.confidenceScore != null ? `${(selectedReq.confidenceScore * 100).toFixed(1)}%` : "—" },
                  { label: "Ngày gửi",        value: formatDate(selectedReq.createdAt) },
                  { label: "Ngày duyệt",      value: formatDate(selectedReq.reviewedAt) },
                  { label: "Ghi chú",         value: selectedReq.note || "—" },
                  { label: "Lý do từ chối",   value: selectedReq.rejectReason || "—" },
                ].map(({ label, value }) => (
              ...
              {/* Ảnh CCCD */}
              <div>
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Ảnh CCCD</h4>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: "Mặt trước", url: selectedReq.idCardFrontSignedUrl },
                    ...
                    <div key={label} className="rounded-xl overflow-hidden border border-gray-200">
                      <div className="px-3 py-2 text-xs font-semibold text-gray-600 bg-gray-50 border-b border-gray-100">{label}</div>
                      {url ? (
                        <a href={url} target="_blank" rel="noopener noreferrer">
                          <img src={url} alt={label} className="w-full object-cover max-h-48 hover:opacity-90 transition-opacity cursor-zoom-in" />
                        </a>
                      ) : (
                        <div className="h-32 flex items-center justify-center text-gray-400 text-xs bg-gray-50">Không có ảnh</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {showRejectInput && selectedReq.status === "PENDING" && (
                <div className="border border-red-200 bg-red-50 rounded-xl p-4 space-y-3">
                  <label className="text-sm font-bold text-red-700">Lý do từ chối *</label>
                  <textarea
                    value={rejectReason}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <div className={`flex flex-col items-center px-6 pt-8 pb-5 ${
              confirmDialog.variant === 'success' ? 'bg-green-50' :
              confirmDialog.variant === 'error'   ? 'bg-red-50' : 'bg-gray-50'
            }`}>
              <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 ${
                confirmDialog.variant === 'success' ? 'bg-green-100' :
                confirmDialog.variant === 'error'   ? 'bg-red-100' : 'bg-gray-100'
              }`}>
              ...
              <h3 className="font-bold text-lg text-gray-900 text-center">{confirmDialog.title}</h3>
              <p className="text-sm text-gray-500 text-center mt-1.5 leading-relaxed">{confirmDialog.message}</p>
            </div>
            ...
            <div className="flex items-center gap-3 px-6 py-4 bg-white">
              ...
              >
                {actionLoading ? 'Đang xử lý...' :
                  confirmDialog.variant === 'success' ? 'Xác nhận Duyệt' : 'Đóng'}
              </button>
            </div>
          </div>
        </div>
