...
function MainApp() {
  ...
  const [filters, setFilters] = useState({ minPrice: "", maxPrice: "", roomType: "", amenities: [] });
    ...
    const fetchRealPosts = async (retryWithoutToken = false) => {
      try {
        ...
        const fetchedRooms = Array.isArray(rawData) ? rawData.map(p => {
          ...
          let imgUrl = "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=800";
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const res = await axios.get(`https://rsapi.goong.io/Geocode?latlng=${latitude},${longitude}&api_key=${GOONG_KEY}`);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <main className="container mx-auto px-4 py-8 relative z-10">
        <div className="mb-10 flex items-end justify-between border-b border-gray-200 pb-5 text-gray-900 relative">
          ...
          <span className="text-sm font-semibold text-rose-500 bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-100">Tìm thấy <span className="font-black text-rose-600">{rooms.length}</span> kết quả</span>
        </div>
